...
Canvas.registerFont(assets.font.get("THEBOLDFONT"), { family: "Bold" })
...
export default [
  {
    ...
    async run({ req }) {
      ...
      try {
        const imageBuffer = await generateWelcomeImageFromURL(
          username.trim(),
          guildName.trim(),
          guildIcon as string,
          parsedMemberCount,
  ...
  },
  {
    ...
    category: "Canvas",
    description:
      "Generate a welcome image (version 1) featuring a user's avatar, username, guild icon, guild name, and member count by uploading image files. This API is designed for applications that require direct file uploads to create engaging visual greetings for new members joining a guild. The image is designed with distinct areas for each element and includes dynamic text formatting. The output is a high-quality JPEG image with adjustable quality.",
          ...
          schema: {
            ...
            properties: {
              ...
              guildIcon: {
                type: "string",
                format: "binary",
                description: "Image file for the guild icon (JPG, JPEG, PNG, GIF, WEBP).",
              },
              memberCount: {
                type: "integer",
                minimum: 0,
    ...
    async run({ req, guf }) {
      ...
      try {
        const imageBuffer = await generateWelcomeImageFromFile(
          username.trim(),
          guildName.trim(),
          guildIconFile.file,
          parsedMemberCount,
