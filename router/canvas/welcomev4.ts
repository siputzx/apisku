...
async function generateWelcomeImageV4FromURL(avatar: string, background: string, description: string): Promise<Buffer> {
...
async function generateWelcomeImageV4FromFile(
  avatarBuffer: Buffer,
...
export default [
  {
    ...
    category: "Canvas",
    description:
      "Generate a customizable welcome image (version 4) using Canvafy. This API allows you to create a personalized welcome banner by providing URLs for the user's avatar and a background image, along with a custom description. It's ideal for use in Discord bots or other community platforms to create engaging visual greetings for new members. The output is a high-quality PNG image.",
    tags: ["CANVAS", "Image Generation", "Welcome Image", "Discord", "Canvafy"],
    example:
      "?avatar=https://i.ibb.co/1s8T3sY/48f7ce63c7aa.jpg&background=https://i.ibb.co/4YBNyvP/images-76.jpg&description=Welcome%20friend!",
    parameters: [
      {
        name: "avatar",
        in: "query",
    ...
    async run({ req }) {
      ...
      try {
        const image = await generateWelcomeImageV4FromURL(avatar.trim(), background.trim(), description.trim())
  ...
  },
  {
    ...
    tags: ["CANVAS", "Image Generation", "Welcome Image", "Discord", "Canvafy", "Upload"],
    example: "",
    requestBody: {
      required: true,
      content: {
    ...
    async run({ req, guf }) {
      ...
      try {
        const image = await generateWelcomeImageV4FromFile(
          avatarFile.file,
          backgroundFile.file,
          description.trim(),
        )
