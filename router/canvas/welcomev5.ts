...
export default [
  {
    ...
    async run({ req }) {
      ...
      try {
        const imageBuffer = await generateWelcomeV5ImageFromURL(
          username.trim(),
          guildName.trim(),
          parsedMemberCount,
          avatar as string,
  ...
  },
  {
    ...
    category: "Canvas",
    description:
      "Generate a stylized welcome image (version 5) by uploading a user's avatar and background image files, along with the username, guild name, and member count. This API is designed for applications that require direct file uploads to create visually appealing welcome banners for new members joining a guild. The image features a prominent circular avatar, a customizable background, and dynamic text sizing for optimal display. The output is a high-quality JPEG image with adjustable quality.",
    tags: ["CANVAS", "Image Generation", "Welcome Image", "Discord", "Stylized", "Upload"],
    example: "",
    requestBody: {
      required: true,
      content: {
    ...
    async run({ req, guf }) {
      ...
      try {
        const imageBuffer = await generateWelcomeV5ImageFromFile(
          username.trim(),
          guildName.trim(),
          parsedMemberCount,
          avatarFile.file,
