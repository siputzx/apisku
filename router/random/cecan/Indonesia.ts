...
async function getRandomCecanIndonesiaImage() {
  try {
    const GIST_URL = "https://raw.githubusercontent.com/siputzx/Databasee/refs/heads/main/cecan/indonesia.json"
    ...
    throw new Error("Failed to get random Indonesian cecan image from API")
  }
}
...
export default [
  {
    metode: "GET",
    endpoint: "/api/r/cecan/indonesia",
    ...
    async run({ req }) {
      try {
        const imageData = await getRandomCecanIndonesiaImage()
