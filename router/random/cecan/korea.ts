...
async function getRandomCecanKoreaImage() {
  try {
    const GIST_URL = "https://raw.githubusercontent.com/siputzx/Databasee/refs/heads/main/cecan/korea.json"
    ...
    throw new Error("Failed to get random Korean cecan image from API")
  }
}
...
export default [
  {
    ...
    async run({ req }) {
      try {
        const imageData = await getRandomCecanKoreaImage()
