...
async function scrapeImageToJpgFromUrl(imageUrl: string) {
...
async function scrapeImageToJpgFromFile(fileBuffer: Buffer, fileName: string) {
  const converter = new CompressImageAPI()
  await converter.getTaskId()
...
export default [
  {
    metode: "GET",
    endpoint: "/api/iloveimg/image2jpg",
    name: "image2jpg",
    category: "Iloveimg",
    ...
    tags: ["ILOVEIMG", "Image Conversion", "JPG"],
    ...
    async run({ req }) {
      ...
      try {
        const convertedImageBuffer = await scrapeImageToJpgFromUrl(image.trim())
        const fileType = await fileTypeFromBuffer(convertedImageBuffer)
        const contentType = fileType ? fileType.mime : "image/jpeg"

        return createImageResponse(convertedImageBuffer, `converted_image.jpg`)
      } catch (error: any) {
  ...
  },
  {
          ...
          schema: {
            ...
            properties: {
              image: {
                type: "string",
                format: "binary",
                description: "The image file to convert to JPG.",
              },
            },
          },
        },
    ...
    async run({ req, guf }) {
      try {
        ...
        const convertedImageBuffer = await scrapeImageToJpgFromFile(fileBuffer, fileName)
