...
async function scrapeSifatUsahaBisnis(tgl: string, bln: string, thn: string) {
  try {
...
export default [
  {
    ...
    async run({ req }) {
      ...
      try {
        const result = await scrapeSifatUsahaBisnis(
          parsedTgl.toString(),
          parsedBln.toString(),
          parsedThn.toString(),
        )
