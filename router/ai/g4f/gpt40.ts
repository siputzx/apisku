...
async function scrapeGpt4o(messages: Message[]) {
  try {
...
export default [
  {
    ...
    tags: ["AI", "Chatbot", "NLP", "GPT-4o", "Advanced"],
    ...
    async run({ req }) {
      ...
      try {
        const result = await scrapeGpt4o([
  ...
  },
  {
    ...
    requestBody: {
      ...
      content: {
        "application/json": {
          schema: {
            type: "array",
            description: "An array of message objects for the GPT-4o model",
            items: {
              type: "object",
              properties: {
                role: {
    ...
    async run({ req }) {
      ...
      try {
        const result = await scrapeGpt4o(messages)
